...
float forward(int hiddenSize, int miniBatch, int seqLength, int numLayers) {
    ...
    // test: memset zero
    cudaErrCheck(cudaMemset(h_data, 0., (seqLength + 1) * (numLayers) * numElements * sizeof(float)));
    cudaErrCheck(cudaMemset(x_data, 0., (seqLength) * (numLayers + 1) * numElements * sizeof(float)));
    cudaErrCheck(cudaMemset(c_data, 0., (seqLength + 1) * (numLayers) * numElements * sizeof(float)));
    cudaErrCheck(cudaMemset(weight, 0., numLayers * hiddenSize * hiddenSize * 8 * sizeof(float)));
    cudaErrCheck(cudaMemset(weight_T, 0., numLayers * hiddenSize * hiddenSize * 8 * sizeof(float)));
    cudaErrCheck(cudaMemset(bias, 0., numLayers * hiddenSize * 8 * sizeof(float)));
    ...
    curandErrCheck(curandGenerateUniform(gen, h_data, (numLayers) * numElements));
    curandErrCheck(curandGenerateUniform(gen, c_data, (numLayers) * numElements));
    // curandErrCheck(curandGenerateUniform(gen, x_data, (seqLength) * numElements));
    ...
    while (true) {
        ...
        for (int layer = lStart; layer < lEnd; layer++) {
            ...
            // tStart, tEnd differ recurBatchSize=2
    ...
    // // test
    // int n = (seqLength) * (numLayers+1) * numElements;
    // float* host = (float*)malloc(n * sizeof(float));
    // cudaMemcpy(host, x_data, n * sizeof(float), cudaMemcpyDeviceToHost);
...
int main(int argc, char* argv[]) {
    ...
    for (int run = 0; run < numRuns; run++) {
        totalTime += forward(hiddenSize, miniBatch, seqLength, numLayers);
    }
