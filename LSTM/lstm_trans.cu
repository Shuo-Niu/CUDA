...
float LSTMTest(int hiddenSize, int miniBatch, int seqLength, int numLayers, bool checkF) {
    ...
    // alloc host memory
    float *x_data_cpu, *weight_cpu, *bias_cpu;
    if(PINNED_MEM) {
        cudaErrCheck(cudaMallocHost((void**)&x_data_cpu, seqLength * numElements * sizeof(float)));
        cudaErrCheck(cudaMallocHost((void**)&weight_cpu, numLayers * hiddenSize * hiddenSize * 8 * sizeof(float)));
        cudaErrCheck(cudaMallocHost((void**)&bias_cpu, numLayers * hiddenSize * 8 * sizeof(float)));
    } else {
        x_data_cpu = (float*)malloc(seqLength * numElements * sizeof(float));
        weight_cpu = (float*)malloc(numLayers * hiddenSize * hiddenSize * 8 * sizeof(float));
        bias_cpu = (float*)malloc(numLayers * hiddenSize * 8 * sizeof(float));
    }
    // printf("Alloc size = %f MB\n", float(seqLength * numElements * sizeof(float) + numLayers * hiddenSize * hiddenSize * 8 * sizeof(float) + numLayers * hiddenSize * 8 * sizeof(float)) / float(1024 * 1024));

    // initialization host values
    srand(0);
    for(int i = 0; i < seqLength * numElements; i++) {
        x_data_cpu[i] = rand() / RAND_MAX;
    }
    for(int i = 0; i < numLayers * hiddenSize * hiddenSize * 8; i++) {
        weight_cpu[i] = rand() / RAND_MAX;
    }
    for(int i = 0; i < numLayers * hiddenSize * 8; i++) {
        bias_cpu[i] = rand() / RAND_MAX;
    }
    ...
    cudaErrCheck(cudaEventCreate(&tstart));
    cudaErrCheck(cudaEventCreate(&tstop));
    cudaErrCheck(cudaEventRecord(tstart));
    // cudaMemcpyHostToDevice x_data, weight, bias
    cudaErrCheck(cudaMemcpy(x_data, x_data_cpu, seqLength * numElements * sizeof(float), cudaMemcpyHostToDevice));
    cudaErrCheck(cudaMemcpy(weight, weight_cpu, numLayers * hiddenSize * hiddenSize * 8 * sizeof(float), cudaMemcpyHostToDevice));
    cudaErrCheck(cudaMemcpy(bias, bias_cpu, numLayers * hiddenSize * 8 * sizeof(float), cudaMemcpyHostToDevice));
    // stop timing data transfer
    cudaErrCheck(cudaEventRecord(tstop));
    cudaErrCheck(cudaEventSynchronize(tstop));
    cudaErrCheck(cudaEventElapsedTime(&transferTime, tstart, tstop));
    cudaErrCheck(cudaDeviceSynchronize());
    printf("Data transfer time: %f ms\n", transferTime);
  
    // LSTM
    const cublasOperation_t a_trans = (PRE_TRANSPOSE && (seqLength > 1)) ? CUBLAS_OP_N : CUBLAS_OP_T;
    // const cublasOperation_t a_trans = CUBLAS_OP_T;
    ...
    cudaErrCheck(cudaMemcpy(x_data_cpu,
